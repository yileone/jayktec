...
public class FusionCharts {
    ...
    private String chartDataSource = "",chartDataSourceEvents= "";
    ...
    public FusionCharts(String type, String id, String width, String height, String renderAt, String dataFormat, String dataSource, String events) {
        this.chartOptions[0] = id;
        this.chartOptions[1] = width;
        this.chartOptions[2] = height;
        this.chartOptions[3] = renderAt;
        ...
        if(this.chartOptions[5].contains("url")) {
            ...
            this.chartOptions[6] = "__dataSource__";
            this.chartOptions[7] = "__events__";
            
            System.out.println(dataSource);
            this.chartDataSource = this.addSlashes(dataSource);
            this.chartDataSourceEvents = this.addSlashes(events);
    ...
    private String jsonEncodeEvents(String[] data){
        String json = "{type: \""+this.chartOptions[4]+"\",renderAt: \""+this.chartOptions[3]+"\",width: \""+this.chartOptions[1]+"\",height: \""+this.chartOptions[2]+"\",dataFormat: \""+this.chartOptions[5]+"\",id: \""+this.chartOptions[0]+"\",dataSource: "+this.chartOptions[6]+",events: "+this.chartOptions[7]+"}";
        return json;
    ...
    public String render() {
        String outputHTML;
        if(this.chartOptions[5].contains("url")) {
            outputHTML = this.constructorTemplate.replace("__constructorOptions__", this.jsonEncodeEvents(this.chartOptions))+this.renderTemplate.replace("__chartId__", this.chartOptions[0]);
        } else {
            if("json".equals(this.chartOptions[5])) {
                outputHTML = this.constructorTemplate.replace("__constructorOptions__", this.jsonEncodeEvents(this.chartOptions).replace("__dataSource__",this.chartDataSource).replace("__events__", this.chartDataSourceEvents))+this.renderTemplate.replace("__chartId__", this.chartOptions[0]);
            } else {
                outputHTML = this.constructorTemplate.replace("__constructorOptions__", this.jsonEncodeEvents(this.chartOptions).replace("__dataSource__","\'"+this.chartDataSource+"\'").replace("__events__", this.chartDataSourceEvents))+this.renderTemplate.replace("__chartId__", this.chartOptions[0]);
            }
        }
